...
function x(){const iter=this.iterable;if(m(iter)){return iter()} else{return function*(){yield* iter}()}}return{next,c,x }}());Collection.prototype[Symbol.iterator]=function*(){let qc;while(!0){qc=this.next();if(qc.done){this.c();break}
yield qc.value}}
...
function e(coll,S=A){J(coll);L(S);const previous=[];return new Collection(function*(){const iter=coll.x();outer:for(let val of iter){inner:for(let prev of previous){if(S(val,prev)){continue outer}}
...
function Concat(gc){J(gc);const cc=this.x();return new Collection(function*(){yield* cc;yield* gc})}
function Union(gc,S=A){J(gc);return this.Concat(gc).Distinct(S)}
function Join(gc,Y,Z,PFn,hc){J(gc);L(Y);L(Z);L(PFn);hc=f(hc,A);L(hc);const fc=this;return new Collection(function*(){for(let fcValue of fc.x()){const fcKey=Y(fcValue);for(let gcValue of gc[Symbol.iterator]()){const gcKey=Z(gcValue);if(hc(fcKey,gcKey)){yield PFn(fcValue,gcValue)}}}})}
function Except(gc){J(gc);if(!p(gc)){gc=new Collection(gc)}
const cc=this.x();return new Collection(function*(){for(let val of cc){if(!gc.Contains(val)){yield val}}})}
function Zip(gc,PFn){J(gc);L(PFn);const cc=this.x();return new Collection(function*(){const dc=gc[Symbol.iterator]();for(let fcVal of cc){const gcNext=dc.next();if(gcNext.done){break}
...
function IndexOf(jcent,S=A){L(S);const iter=this.x();let i=0;for(let val of iter){if(S(val,jcent)){return i}
i++}
return -1}
function LastIndexOf(jcent,S=A){L(S);const iter=this.x();let i=0;let lastIndex=-1;for(let val of iter){if(S(val,jcent)){lastIndex=i}
i++}
return lastIndex}
function Contains(jc,S=A){return !!~this.IndexOf(jc,S)}
function Where(aa=(jc,index)=>!0){L(aa);const iter=this.x();const result=new Collection(function*(){let index=0;for(let val of iter){if(aa(val,index)){yield val}
index++}});return result}
...
const iter=this.x();return new Collection(function*(){let i=0;for(let val of iter){yield val;if(++i===count){break}}})}
...
function TakeWhile(aa=(jc,index)=>!0){L(aa);const iter=this.x();const result=new Collection(function*(){let index=0;let endTake=!1;for(let val of iter){if(!endTake&&aa(val,index++)){yield val;continue}
endTake=!0}});this.c();return result}
function TakeUntil(aa=(jc,index)=>!1){return this.TakeWhile((jc,index)=>!aa(jc,index))}
function SkipWhile(aa=(jc,index)=>!0){L(aa);const iter=this.x();return new Collection(function*(){let index=0;let endSkip=!1;for(let val of iter){if(!endSkip&&aa(val,index++)){continue}
endSkip=!0;yield val}})}
...
function Single(aa=x=>!0){L(aa);I(this);let index=0;let result;for(let val of this.x()){if(aa(val)){result=val;break}
index++}
...
function Select(mapFn=x=>x){const iter=this.x();let index=0;return new Collection(function*(){for(let val of iter){yield mapFn(val,index);index ++}})}
function Flatten(){return this.SelectMany(x=>x)}
function SelectMany(mapFn,P=(x,y)=>y){L(mapFn);L(P);const iter=this.x();return new Collection(function*(){let index=0;for(let qc of iter){let pc=mapFn(qc,index);let newIter=pc;if(!l(pc)){newIter=[pc]} else{newIter=pc}
...
return lc(a,b)};
return this};
OrderedLinqCollection.prototype.x=function(){const _self=this;return function*(){yield* Reflect.construct(_self.__heapConstructor,[[..._self.iterable],_self.__comparator])}()}
...
function y(groups,key,U){for(let groupKey of groups.keys()){if(U(groupKey,key)){return groupKey}}
return key}
...
function s(T,inner){let U,V;if(k(inner)){U=inner;V=jc=>jc} else{U=A;V=inner}
return t(T,V,U)}
function t(T,inner,third){let U,V,P;if(k(third)){U=third} else{P=third}
if(w(inner)===2){P=inner} else{V=inner}
if(!U){U=A}
if(!V){V=jc=>jc}
return v(T,V,P,U)}
function v(T,V,P,U){L(T);L(V);O(g(P)||n(P),'P must be undefined or function!');L(U);let groups=new Map();let result;for(let val of arr){const key=y(groups,T(val),U);const jc=V(val);if(groups.has(key)){groups.get(key).push(jc)} else{groups.set(key,[jc])}}
...
return fn(T,...args)}
function GroupJoin(inner,outerKeySelector,innerKeySelector,P,S=A){J(inner);L(outerKeySelector);L(innerKeySelector);L(P);let groups=new Map();const outer=this;for(let outerVal of outer.x()){const outerKey=outerKeySelector(outerVal);groups.set(outerVal,new Collection(function*(){for(let innerVal of inner[Symbol.iterator]()){if(S(outerKey,innerKeySelector(innerVal))){yield innerVal}}}))}
