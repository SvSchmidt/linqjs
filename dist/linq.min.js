...
function Join(gc,Y,Z,PFn,hc){J(gc);L(Y);L(Z);L(PFn);hc=f(hc,A);L(hc);const fc=this;return new Collection(function*(){for(let fcValue of fc.y()){const fcKey=Y(fcValue);for(let gcValue of gc[Symbol.iterator]()){const gcKey=Z(gcValue);if(hc(fcKey,gcKey)){yield PFn(fcValue,gcValue)}}}})}
...
return fn(T,...args)}function GroupJoin(gc,Y,Z,P,S=A){let result=[];for(let fcVal of this.y()){}}
