...
function x(){const iter=this.iterable;if(m(iter)){return iter()}else{return function*(){yield* iter}()}}return{next,c,x }}());Collection.prototype[Symbol.iterator]=function*(){let ud;while(!0){ud=this.next();if(ud.done){this.c();break}
yield ud.value}}
...
function d(arr,value){N(arr);let xcsBefore=[];let xcFound=!1;let ud;while((ud=arr.shift())&&!(xcFound=A(ud,value))){xcsBefore.push(ud)}
...
function SkipWhile(jc=(xc,index)=>!0){L(jc);const _wc=this;return new Collection(function*(){let index=0;let yd=!1;for(let val of _wc.x()){if(!yd&&jc(val,index++)){continue}
yd=!0;yield val}})}
...
return [z(U)]}
let zd=(function(){function zd(index,value){this.__index=index;this.__value=value;this.__iszdInstance=!0}
zd.Createzd=function(index,ac){if(ac===undefined||ac.__iszdInstance){return ac}
return new zd(index,ac)};return zd})();let MinHeap=(function(){function MinHeap(kc,gc=defaultComparator){N(kc);L(gc);this.kc=kc;this.gc=(a,b)=>{let res=gc(a.__value,b.__value);if(res!==0){return res}
return defaultComparator(a.__index,b.__index)};B(this.kc,this.gc)}
function r(kc,gc,i){let rd=2*(i+1);let qd=rd-1;let bestIndex=i;kc[bestIndex]=zd.Createzd(bestIndex,kc[bestIndex]);if(qd<kc.length){kc[qd]=zd.Createzd(qd,kc[qd]);if(gc(kc[qd],kc[bestIndex])<0){bestIndex=qd}}
if(rd<kc.length){kc[rd]=zd.Createzd(rd,kc[rd]);if(gc(kc[rd],kc[bestIndex])<0){bestIndex=rd}}
...
MinHeap.prototype.ad=function(){return this.kc.length>0};MinHeap.prototype.getTopElement=function(){if(this.kc.length===1){return this.kc.pop().__value}
let topElement=this.kc[0];let tmp=this.kc.pop();this.kc[0]=tmp;r(this.kc,this.gc,0);return topElement.__value};MinHeap.prototype[Symbol.iterator]=function(){let heap=this;return{next:function(){if(heap.ad()){return{done:!1,
...
function SelectMany(tc,cc=(x,y)=>y){L(tc);L(cc);const iter=this.x();return new Collection(function*(){let index=0;for(let ud of iter){let td=tc(ud,index);let newIter=td;if(!l(td)){newIter=[td]}else{newIter=td}
for(let val of newIter[Symbol.iterator]()){yield cc(ud,val)}
index++}})}
...
OrderedCollection.prototype.ThenBy=function(ec,gc=defaultComparator){const udComparator=this.__gc;const sd=GetComparatorFromKeySelector(ec,gc);const newComparator=(a,b)=>{const res=udComparator(a,b);if(res!==0){return res}
return sd(a,b)}
...
function y(pc,key,dc){for(let xd of pc.keys()){if(dc(xd,key)){return xd}}
return key}
